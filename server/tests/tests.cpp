...
// För att det ska kompilera korrekt behöver du lägga till alla .cpp filer som
// ditt test använder i server/CMakeLists.txt add_executables(tests...) (rad 96) Observera ej
// server/tests/Cmakelists

// Testfilen körs genom python3 run_tests.py från terminalen
...
TEST_CASE("extract creates a .wav file from video input", "[extract]") {
  ...
  // Verifiera att filen skapats korrekt i user_pathen
...
/* Videoanalys testning */

TEST_CASE("Testar att analyse_video() ger en rimlig output, rätt mängd rörelsetillfällen"){
  std::string video_file = "sample_video.mp4";
 ...
 if(!std::filesystem::exists(user_path)){
  ...
  std::vector<int> coords = {1, 200, 1, 200};
  std::vector<int> result = analyse_video(user_path, coords);

  REQUIRE(result.size() == 4);

}

TEST_CASE("Kastar undantag om filväg ej hittas", "[error]"){
  std::vector<int> valid_coords = {1, 200, 1, 200};
  REQUIRE_THROWS_AS(analyse_video("", valid_coords), std::invalid_argument);
}
...
TEST_CASE("Kastar undantag om ROI är utanför bilden", "[error]") {
  ...
  REQUIRE_THROWS_AS(analyse_video("sample_video.mp4", invalid_coords), std::invalid_argument);
}
