...
// Tid som det ska vara still för att avsluta motion
#define STOP_IF_STILL_FOR 0.5

// Andel pixlar ändrade för motion
#define MOTION_THRESHOLD 0.001
...
std::vector<int> analyse_video(std::string path, std::vector<int> cords) {
    ...
    while (true) {
        ...
        if (motion_pixels > w * h * MOTION_THRESHOLD) {  // Finjustera
        if (!in_motion) {
            // Starta rörelse
